...
CLASS_LABELS = {
    0: 'BETA-DOMINANT EEG (Class 0)',
    1: 'NON-BETA / ARTIFACT (Class 1)'
}
...
def load_and_preprocess_file(filepath, time_scaler, stat_scaler, freq_scaler, freq_feat_size):
    ...
    # Handle multi-column
    if data.ndim == 2:
        print(f"  Multi-column data detected: {data.shape}")
        if data.shape[1] == 2 and np.allclose(data[:, 0], data[:, 1]):
            print("  Using first column (duplicate detected)")
            data = data[:, 0]
        else:
            data = data[:, 0]
    ...
    if len(data) >= NUM_SAMPLES:
        ...
        print(f"  Padding from {len(data)} to {NUM_SAMPLES} samples")
    ...
    for i, name in enumerate(bands_names):
        rel_power = band_powers[name.lower()] / total_power * 100
        print(f"    {name:8s}: {rel_power:5.2f}%")
...
def main():
    ...
    print("="*80)
    print("BETA vs NON-BETA EEG CLASSIFIER - TEST MODE")
    print("="*80)
    
    # Detect feature sizes from model
    print("\nDetecting model architecture...")
    stat_feat_size, freq_feat_size = detect_model_feature_sizes('beta_classifier_best.pth')
    print(f"  Statistical features: {stat_feat_size}")
    print(f"  Frequency features: {freq_feat_size}")
    ...
    print(f"\nRaw Probabilities:")
    print(f"  Class 0 (Beta): {(1-raw_prob)*100:.2f}%")
    print(f"  Class 1 (Non-Beta): {raw_prob*100:.2f}%")
    
    print("\n" + "-"*80)
    print("INTERPRETATION:")
    print("-"*80)
    
    beta_power_pct = band_powers['beta'] / total_power * 100
    
    if predicted_class == 0:
        print("✓ This signal is classified as BETA-DOMINANT EEG")
        print(f"  Beta band power: {beta_power_pct:.2f}%")
        print("  This indicates an alert, focused brain state")
        if confidence > 0.9:
            print("  High confidence - strong beta characteristics detected")
        elif confidence > 0.7:
            print("  Good confidence - clear beta pattern")
        else:
            print("  Moderate confidence - some ambiguity in signal")
    else:
        print("⚠ This signal is classified as NON-BETA / ARTIFACT")
        print(f"  Beta band power: {beta_power_pct:.2f}%")
        
        # Identify dominant band
        max_band = max(band_powers.items(), key=lambda x: x[1])
        max_band_pct = max_band[1] / total_power * 100
        print(f"  Dominant band: {max_band[0].upper()} ({max_band_pct:.2f}%)")
        
        if max_band[0] == 'alpha':
            print("  Suggests relaxed/resting state")
        elif max_band[0] == 'theta':
            print("  Suggests drowsy/meditative state")
        elif max_band[0] == 'delta':
            print("  Suggests deep sleep or possible artifacts")
        elif max_band[0] == 'gamma':
            print("  Suggests muscle artifacts or noise")
