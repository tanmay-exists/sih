...
export const LoginPage = ({ onLogin }) => {
  ...
  const handleGoogleSSO = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/auth/google?role=${role}`);
      window.location.href = response.data.authorization_url;
    } catch (err) {
      setError("Failed to initiate Google SSO. Try again.");
    }
  };
      ...
      >
        ...
        <div className="relative z-10 text-center max-w-lg w-full mx-auto">
          ...
          >
            ...
            >
              {/* <svg className="h-7 w-7 mr-2" viewBox="0 0 24 24"> */}
              {/*   <path */}
